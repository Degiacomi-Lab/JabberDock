...
class CMAViEParameters(object):
    
    def __init__(self, N, opts, population, ccovfac=1, verbose=True):
        """Compute strategy parameters, mainly depending on
        dimension and population size, by calling `set` 
        
        """
        self.N = N
        self.Vpop = population
    ...
    def set(self, opts, popsize=None, ccovfac=1, verbose=True):
        ...
        sp.popsize = popsize   # __CHANGE_POPSIZE__ here and in default CMAParameters.py
        ...
        sp.wholePopFlag = True
        wholePopFlag = sp.wholePopFlag
        ...
        # mu depends on mu_f so should not be touched
        if 1 < 3:
            sp.mu = int(sp.mu_f + 0.499999) # round down for x.5
        else:
            sp.mu = int(Vpop)
        
#        print ("sp.mu: "+str(sp.mu))
...
#            print (log(max([sp.mu, sp.popsize]) + 0.5))
#            print (len(sp.weights))

        elif 1< 3 and not sp.wholePopFlag:
#            sp.weights = log(sp.mu + 0.5) - log(1 + np.arange(sp.mu))
            sp.weights = log(sp.mu + 0.5) - log(1 + np.arange(sp.mu))
            weigh = copy.deepcopy(sp.weights)
            weightedSum = (sum(weigh))
            sp.weights = np.ones(sp.mu)
#            sp.weights[0:sp.mu] = log(sp.mu + 0.5) - log(1 + np.arange(sp.mu))
            # recalculate the weight based on the survivors:
            
#            print Vpop
            if Vpop <= sp.mu and Vpop != 0:
                modulator = float(weightedSum / sp.Vpop)
#                print (">> Vpop: "+str(Vpop))
                for i in xrange(0, sp.mu, 1):
                    if i < Vpop:
                        sp.weights[i] = 1
#                        sp.weights[i] = weigh[i]
#                        sp.weights[i] = (weigh[i] * modulator)
...
#                sp.weights = weigh
        ...
        if sp.wholePopFlag:
            sp.weights = log(sp.mu) - log(1 + np.arange(sp.mu))
#            sp.weights = log(sp.mu + 0.5) - log(1 + np.arange(sp.mu))
            weigh = copy.deepcopy(sp.weights)
            
            sp.weights = np.ones(sp.popsize)
            for i in xrange(0, sp.popsize, 1):
                if i < Vpop:
                    sp.weights[i] = 1
#                    sp.weights[i] = weigh[i]
#                        sp.weights[i] = (weigh[i] * modulator)
