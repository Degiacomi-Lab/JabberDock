...
class LoggingFunction(object):
    ...
    * data files (:file:`dat`, :file:`tdat` extensions) which record
    ...
    def _readytostart(self):
        ...
        # -------------- OPENING INDEX FILE
        comm.Barrier()
        self.res = comm.gather(res[-1], root = 0)
        comm.Barrier()


        if rank == 0 and len(res) != 1: # __NUMBER1__
            f = open(self.indexfile, 'a')
            res.pop(-1)
            f.writelines(res) # does not add line separator __WRITE__
            f.close()
...
#            if rank == 0: # __NUMBER2__ ---------------------------------------- DATAFILE

            comm.Barrier()
            self.fopt_array = comm.gather(self.fopt)
            comm.Barrier()

#            f = open(datafile, 'a') # __WRITE__
...
#            f.close()
        ...
        # added variables for the parallelisation:
        self.buffr = []
        self.hbuffr = []
    ...
    def evalfun(self, inputx, *args, **kwargs):
        ...
        if (self.lasteval.num + popsi >= self.evalsTrigger or
            ...
            #
#            if rank == 0: # __NUMBER3__

            # insert elements in the buffr and hbuffr arrays:
            self.buffr.append( buffr )
            self.hbuffr.append(hbuffr)

#            if buffr:
#                f = open(self.datafile, 'a')
#                f.writelines(buffr) # __WRITE__ ---------------------------------------- DATAFILE
#                f.close()
#            if hbuffr:
#                f = open(self.hdatafile, 'a')
#                f.writelines(hbuffr) # __WRITE__ ---------------------------------------- DATAFILE
#                f.close()
        else:
    ...
    def finalizerun(self):
        ...
        if self._is_finalized:
            ...
            return
        self.lasteval.is_written = False # __NEW__ added this as the file was not being written otherwise
        if not self.lasteval.is_written:
            print "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX "+str(self.datafile)+"XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX "
...
#            if rank == 0: # __NUMBER4__ ---------------------------------------- DATAFILE

            # get all the data written previously:
            comm.Barrier()
            buffr_array = comm.gather( self.buffr )
            hbuffr_array = comm.gather( self.hbuffr)
            lasteval_array = comm.gather(self.lasteval.sprintData(self.fopt))
            comm.Barrier()

#            if rank == 0:
#                print "buffr "+str(buffr_array)+"\n\n"
#                print "hbuffr "+str(hbuffr_array)+"\n\n"
#                print "self.fopt arr "+str(self.fopt_array)+"\n\n"
#                print "lasteval arr "+str(lasteval_array)+"\n\n"
#                ccc
#            print self.lasteval.sprintData(self.fopt)
#            ccc
...
#            f.write(self.lasteval.sprintData(self.fopt))
            ...
            if rank == 0:
                ...
                # ----------------------------- WRITING TDATA FILE -----------------------------
                f = open(self.datafile, 'a')
                for i in xrange(0, len(self.fopt_array), 1):
                    f.write('%% function evaluation | noise-free fitness - Fopt'
                            ' (%13.12e) | best noise-free fitness - Fopt | measured '
                            'fitness | best measured fitness | x1 | x2...\n'
                            % self.fopt_array[i])
                    # write then all the lines:
                    for txt in buffr_array[i]:
                        if txt:
    #                        f = open(self.datafile, 'a')
                            f.writelines(txt)
    #                        f.close()
    ...
    #                        f.close()
                    ...
                    f.write(lasteval_array[i])
                f.close()

                # ----------------------------- WRITING HDATA FILE -----------------------------
                f = open(self.hdatafile, 'a')
                for j in xrange(0, len(self.fopt_array), 1):
                    f.write('%% function evaluation | noise-free fitness - Fopt'
                            ' (%13.12e) | best noise-free fitness - Fopt | measured '
                            'fitness | best measured fitness | x1 | x2...\n'
                            % self.fopt_array[j])
                    for htxt in hbuffr_array[j]:
                        f.writelines(htxt)
                f.close()

                # ----------------------------- WRITING RDATA FILE -----------------------------
                f = open(self.rdatafile, 'a')
                for r in xrange(0, len(self.fopt_array), 1):
                    f.write('%% function evaluation | noise-free fitness - Fopt'
                            ' (%13.12e) | best noise-free fitness - Fopt | measured '
                            'fitness | best measured fitness | x1 | x2...\n'
                            % self.fopt_array[r])
        ...
        # get the lasteval.num of every processor
        lasteval_num = int(self.lasteval.num)
        lasteval_bestf = float( self.lasteval.bestf)
        fopt = float (self.fopt)

        comm.Barrier()
        lasteval_num = comm.gather(lasteval_num)
        lasteval_bestf = comm.gather(lasteval_bestf)
        fopt = comm.gather(fopt)
        comm.Barrier()

        if rank == 0: # __NUMBER5__ only writes the best next to the number

#            comm.Barrier()
#            lasteval_num = comm.bcast(lasteval_num,root=0)
#            lasteval_bestf = comm.bcast(lasteval_bestf,root=0)
#            fopt = comm.bcast(fopt,root=0)
#            comm.Barrier()
#            print "self.res "+str(self.res)
#            print "lastval num"+str(lasteval_num)
#            print "lastval bestf "+str(lasteval_bestf)
##            print "lastval bestf min "+str(min(lasteval_bestf))
#            print "fopt "+str(fopt)
#
            f = open(self.indexfile, 'a')
            for i in xrange(0, len(lasteval_num),1):
                f.write('%s:%d|%.1e' % (self.res[i],sum(lasteval_num), lasteval_bestf[i] - fopt[i] - self.precision)) # __WRITE__
#            f.write(':%d|%.1e' % (self.lasteval.num, self.lasteval.bestf - self.fopt - self.precision)) # __WRITE__
            f.close()
        self._is_finalized = True
