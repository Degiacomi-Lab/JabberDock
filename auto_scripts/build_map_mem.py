...
# note that this script is actually redundant with build_map, but is present for completeness / to avoid confusion
...
parser.add_argument('-i', metavar="input", required=True, help='Input simulation multipdb (e.g. sim.pdb from gmx_mem_run)')
parser.add_argument('-ff_dat', metavar="forcefield", required=True, help='Forcefield charge data used to run the MD. This is a required option, but one is avaliable in biobox/classes/amber03 to correspond to the amber03 forcefield used in gmx_run. Note you only need a file for the protein part of your forcefield (not lipid).')
...
parser.add_argument('-te', metavar="time_end", default=900, required=False, help='Frame from which to stop building the STID maps from (default is 900)')
